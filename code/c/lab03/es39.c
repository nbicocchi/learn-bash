...
int child(struct test_pipe *t, pipe_t *p, int child_n, int child_id) {
    ...
    for (i = 0; i < child_n; i++) {
        ...
        if ((i != child_id - 1) && (child_id != 0)) close(p[i][0]);
    }
    
    /* the first child activates the pipeline */
    if (child_id == 0) {
        zprintf(1, "[%d] writing to the next child...\n", getpid());
    ...
    /* wait for command */          
    if (read(p[child_id - 1][0], &cmd, 1) != 1) {
